...
local function SaveIup()
    ...
    if pcall(io.output, file) then
        ...
        iup.Alarm("HidlM", "Ќевозможно сохранить настройки в файл Settings.lua!", "Ok")
    end
    io.close()
    iup.SaveChProps()
end
...
iup.CloseFilesSet = function(cmd, tForClose)
    ...
    for i = 0,maxN do
        ...
        if not scite.buffers.SavedAt(i) and MastClose(i) and (cmd ~= 9134 or pth:find('Ѕезым€нный')) and not fnExt:find('^%^') then
...
iup.scitedetachbox = function(t)
  ...
  --[[      hNew.title= t.Dlg_Title or "dialog"]]
    ...
    if t.MenuEx then menuhandler:InsertItem(t.MenuEx, 'xxxxxx', {'View', ru = '¬ид', visible = t.MenuVisibleEx, tSub}) end
...
iup.ConfigList = function()
    if not mnu_configs then
        ...
        table.insert(mnu_configs, mnu_i)
        mnu_i = {'Load...', ru = '«агрузить...', action=LoadIuprops, image = 'folder_open_document_µ'}
        table.insert(mnu_configs, mnu_i)
    end
    return mnu_configs
end
