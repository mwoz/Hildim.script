...
Вид комментария зависит от выделения текста:
  ...
  * Если параметры comment.stream.* отсутствуют, то независимо от способа выделения ставится|снимается блочный комментарий.

Снятие/установка комментария зависит от первого символа выделения:
  * Если первый символ в выделении закомментирован, то комментарий снимается со всего выделения.
...
-- Комментирование одной невыделенной строки
	...
	if comment_block == "" then
		...
		if comment_block_at_line_start == 44 then
...
-- Снятие комментария с одной невыделенной строки
	...
	if comment_block == "" then
		...
		local line_pos_end = editor:PositionFromLine(line_sel_start + 1)  --line_pos_start + string.len(text_line)
...
-- Обработка нажатия на Ctrl+Q
    if editor.Lexer  == SCLEX_FORMENJINE then
        ...
        if sector == SCE_FM_VB_DEFAULT then
            props['comment.block.formenjine'] = "'"
            props["comment.stream.start.formenjine"] = ''
            props["comment.stream.end.formenjine"] = ''
	...
	if sel_text == "" and comment_block ~= '' then
		-- This Line
	...
	if IsBlock() and comment_block ~= '' then
	-- This Block
        if IsComment(FirstLetterFromBlock()) then
            BlockUnComment()
        else
