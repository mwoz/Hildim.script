...
local win_parent --ÒÓÁ‰‡ÂÏ ÓÒÌÓ‚ÌÓÂ ÓÍÌÓ
local tbs
local vbox
...
local function  CreateBox()
    ...
    local function SideBar(t, Bar_Obj, sciteid)
        ...
        t.tabchange_cb = (function(_,new_tab, old_tab)
            --ÒÌ‡˜‡Î‡ Ì‡È‰ÂÏ ‡ÍÚË‚Ì˚È Ú‡· Ë ÛÒÚ‡ÌÓ‚ËÏ Â„Ó ‚ SideBar_ob
            for _,tbs in pairs(SideBar_Plugins) do
...
local function InitSideBar()
    ...
    -- SideBar_obj.win = false --ÂÒÎË ÛÒÚ‡ÌÓ‚ËÚ¸ true - Ô‡ÌÂÎ¸ ·Û‰ÂÚ ÔÓÍ‡Á‡Ì‡ ÓÚ‰ÂÎ¸Ì˚Ï ÓÍÌÓÏ
    SideBar_Plugins = {}
...
AddEventHandler("OnSendEditor", function(id_msg, wp, lp)
    if id_msg == SCN_NOTYFY_ONPOST then
        if wp == POST_CONTINUESTARTUP then  --œÓÍ‡Á ÓÚ‰ÂÎ¸Ì˚Ï ÓÍÌÓÏ ‡Á‚ˇÁ˚‚‡ÂÏ ˜ÂÂÁ ÔÓÒÚ, ËÌ‡˜Â ÔÎÓıÓ ËÍÓÌÍË ÔÓÍ‡Á˚‚‡ÂÚ
