...
_G.sys_Menus.MainWindowMenu = {title = _TM"Main Window Menu",
	...
	{'&Tools', {
		{'&Compile', key = 'F6', action = IDM_COMPILE, image = 'compile_µ', visible = function() if props["command.compile$"]~= "" then local s = props["command.compile.name$"] return Iif(s~= "", s, true) end end},
		{'&Build', key = 'F7', action = IDM_BUILD, image = 'building__arrow_µ', visible = function() if props["command.build$"]~= "" then local s = props["command.build.name$"] return Iif(s~= "", s, true) end end},
		{'&Run', key = 'F5', action = IDM_GO, image = 'control_µ', visible = function() if props["command.go$"]~= "" then local s = props["command.go.name$"] return Iif(s~= "", s, true) end end},
