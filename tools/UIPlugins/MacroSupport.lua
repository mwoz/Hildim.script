...
local function Init_hidden()
    ...
    local item = {'Macros', cpt = _T'Macro', {
		...
		{'Delete Current', cpt = _T'Delete Current Macro', action = function() if iup.Alarm("Macros", _T'Delete Macro?', _TH'Да', _TH'Нет') == 1 then recordet_macros = nil end end, visible = function() return not MACRO.Record and recordet_macros end, image = "cross_script_µ"},
