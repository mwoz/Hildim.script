...
local function ShowExpansionList(event_IDM_ABBREV)
	...
	for i = 1, #table_abbr_exp do
		if table_abbr_exp[i].abbr:find(abbrev, 1, true) == 1 then
			--слишком длинные строки обрежем и дополним пробелами
			local strItem = table_abbr_exp[i].exp
			if #strItem > 55 then strItem = (strItem:sub(1,50)).."..." end
			table_user_list[#table_user_list+1] = {table_abbr_exp[i].abbr, strItem}
		end
	end
